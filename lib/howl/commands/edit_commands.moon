...
command.register
  ...
  input: (opts)->
    opts.help\add_keys
      ctrl_r: 'Switch to <command_name>buffer-replace-regex</>'
    buffer = app.editor.buffer
    selection = selected_chunk!
    replacement = interact.buffer_replace
      prompt: opts.prompt
...
command.register
  ...
  input: (opts) ->
    opts.help\add_keys
      ctrl_h: 'Switch to <command_name>buffer-replace</>'
    buffer = app.editor.buffer
    selection = selected_chunk!
    replacement = interact.buffer_replace
      prompt: opts.prompt
      ...
      parse_query: (query) ->
        ...
        error('Invalid regex', 0) unless status
        return query
      find: (text, query, start) ->
        match = table.pack query\find text, start
        captures = [match[idx + 2] for idx = 1, (query.capture_count or 0)]
